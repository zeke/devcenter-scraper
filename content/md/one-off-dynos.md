...
`heroku run` takes two types of parameters - you can either supply the command to execute, or a dyno type that is present in your application's [Procfile](procfile).
...
<div class="callout" markdown="1">To pass command line flags to the command being executed, you can quote the entire string to be executed to avoid the Heroku CLI processing the flags: `heroku run "rake --help"`</div>
...
<div class="callout" markdown="1">It is possible to trap `SIGHUP` and cause your dyno to continue running even when the connection is closed. See the [signal manual page](http://man.cx/signal%287%29) for more information.</div>
...
By default, one-off dynos run at the [1X size](https://devcenter.heroku.com/articles/dyno-size). You can run one-off dynos as a [2X dyno](https://devcenter.heroku.com/articles/dyno-size) by passing the size flag:
